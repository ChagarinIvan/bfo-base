...
- [ ] T004 [US1] **Изоляция БД-тестов** (чинит засорение dev-БД, жалоба №1; готовит параллель-безопасность):
  привести все БД-тесты к транзакционному откату/изоляции — добавить `RefreshDatabase` (или общий
  DB-базовый TestCase) там, где тест обращается к БД без него; убедиться, что serial-прогон не оставляет
  строк в `bfo_base`; гейты; коммит
- [ ] T005 [US1] **Параллельный прогон**: `collision`+`paratest` уже установлены (dev) и выдан грант `dev`
  на `bfo_base_test_%`; отладить per-process DB-switch (интеграция кастомного `bootstrap/app.php`/`TestCase`
  с `ParallelTesting` — чтобы запросы шли в `bfo_base_test_N`, а не в `bfo_base`); включить
  `php artisan test --parallel` в скрипт `test`; убедиться в отсутствии гонок/коллизий
- [ ] T006 [US1] Гигиена фикстур: где БД не нужна — `make()`/стабы вместо `create()`; сократить
  персистируемые строки в тестах-«тяжеловесах»; повторный замер (**медиана из 3**) заметно меньше базлайна
  (~193с); прогон зелёный и стабильный; гейты; коммит
