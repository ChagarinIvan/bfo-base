...
- Изначальные дефолты закрыли спорные места (MySQL для тестов, определение легаси через слои конституции).
  На `/clarify` (2026-08-27) уточнены две развилки: SC-001 — без числового порога (оценка сравнением
  замеров до/после); US3 — широкий охват целевого слоя (не только ranks/cups).
