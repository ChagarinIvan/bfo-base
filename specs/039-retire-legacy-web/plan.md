...
Preserve the existing authenticated users API used by SPA impressions. Resolve authors by email and refresh the cached user list when an impression references an ID absent from that list. The manual cup cache action flushes the shared `cups` tag without a cup ID; queued invalidation uses the same operation and never checks cup active state.
...
| Commands and mutation | Actions with input construct commands and call `execute()`. Global cache clear has no input and calls its Application service directly. Existing disable and cache services remain the mutation authorities. |
...
8. Confirm `/api/v1/users` remains authenticated and returns the existing limited user DTO; update the shared impression label and stale-list recovery in the SPA.
