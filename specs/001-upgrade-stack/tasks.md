...
- [ ] T001 Убедиться, что работа идёт в ветке `001-upgrade-stack` (создать от `master`, если ещё нет)
- [ ] T002 Зафиксировать базовую линию: прогнать `composer install`, `composer test`, `composer stan`, `composer cs`; записать результат. Любой предсуществующий «красный» гейт починить ДО начала апгрейда
