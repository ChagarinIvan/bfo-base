...
> ⚠️ **ПЕРЕСМОТРЕНО (2026-08-19, T010):** выбран обратный путь — стандартизация на **predis**, а не
> phpredis. Причина: phpredis не установлен ни в одном окружении, а `.env`/`.env.example` реально
> используют predis → переход на phpredis = изменение поведения + установка C-расширения везде. Вместо
> удаления predis подняли `1.1.10 → 3.6.0` (`^2.3 || ^3.0`; L13 официально поддерживает `^2.3 || ^3.0`,
> predis 3.x работает на PHP 8.5). Проверено на живом redis. См. T010 в tasks.md. Блок ниже — исходное
> (отвергнутое) решение, оставлен для истории.
