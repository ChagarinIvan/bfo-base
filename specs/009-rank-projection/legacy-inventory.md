...
| `app/Domain/Rank/Rank.php` и `app/Domain/Rank/RankRepository.php` | заменить aggregate enum-ом и удалить контракт | string-backed enum `Domain\\Rank\\Rank` | выполнено |
...
| `app/Application/Service/Rank/RefillPersonRanksService.php` | перевести и удалить оболочку | `RebuildPersonRanksService` + `ranks:refill` | удалено |
...
| `resources/views/ranks/*`, navbar и translations | сохранить только поддерживаемую историю | новый history read use case | выполнено; старый список убран из navbar |
| `app/Repositories/ProtocolLinesRepository.php` | не использовать в новом rank flow | `RankFacts` + `EloquentRankFacts` | остаётся legacy dependency `ProtocolLineService`, вне rank flow |
