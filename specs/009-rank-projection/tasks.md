...
- [x] T054 Уточнить проекцию периодов при повышении: завершать активные более низкие периоды датой повышения, маркировать последующее выполнение низшего в действующем высшем периоде как `lower_rank_confirmation` и отложить начало его следующего периода до истечения высшего; покрыть `tests/Domain/Rank/RankCalculatorTest.php`.
