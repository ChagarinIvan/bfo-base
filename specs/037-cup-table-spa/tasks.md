...
- [X] Перевести CupView на родительский `CupLayoutPage` с вкладками событий и таблицы, как у PersonLayoutPage.
- [X] Всегда рендерить обязательные колонки этапов, даже если сохранённая настройка таблицы их не содержит.
- [X] Указать поля `place`, `personYear`, `totalPoints` и `averagePoints` для стандартных PrimeVue колонок.
- [X] Установить размер страницы таблицы по умолчанию 20 и убрать колонку клуба.
- [X] Упорядочить карточные действия и удалить экспорт из области над таблицей.
- [X] Показывать в колонках этапов только дату, а название — в подсказке; кэшировать запрос событий по `ids` на 24 часа.
- [X] Оформить зачётные ячейки мягким зелёным, незачётные мягким красным фоном.
