...
## Decision: Read activated same-rank protocol lines with an existence query

**Rationale**: The source of activation is `protocol_lines.activate_rank`. An existence
query scoped to athlete, canonical rank, non-null activation and another protocol-line ID
answers the rule without trusting a potentially stale derived rank-history projection.
...
- Load every previous line into PHP: rejected because a database existence query is
