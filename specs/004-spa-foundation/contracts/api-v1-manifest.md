...
**Статус**: производный справочный контракт; обязательные правила находятся в
...
- Общий API action adapter автоматически читает `#[HttpError]` и формирует
  `{ "errors": [{ "code": "...", "message": "..." }] }`.
- Ожидаемая HTTP-ошибка нового API обязана наследоваться от
  `ApplicationException` и иметь `#[HttpError]`: тогда `ApiAction` возвращает
  осознанный 4xx JSON-ответ до глобального exception handler. Такие ответы не
  репортируются в Sentry; непредвиденные 5xx и ошибки очередей/cron остаются
  наблюдаемыми.
...
Пользовательский интерфейс SPA поддерживает только белорусский язык. Русская
локализация для SPA не поддерживается. Тексты не зашиваются в Vue-шаблоны:
они берутся по ключам из существующего словаря `resources/lang/by.json` через
типизированный адаптер `resources/spa/i18n.ts`.
