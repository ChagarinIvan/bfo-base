...
import { t } from '../../i18n'
import type { ApiResponse, Competition, Impression, User } from '../../api/types'
...
function userLabel(impression: Impression | undefined): string {
  ...
  return user?.name || user?.email || t('spa.competitions.unknown_user', { id: impression.by })
}
...
async function load(): Promise<void> {
  ...
  error.value = ''
  try {
    competitions.value = (await api.get<ApiResponse<Competition[]>>('/competitions', { params: { year: year.value } })).data.data

    if (auth.isAuthenticated && users.value.length === 0) {
      users.value = (await api.get<ApiResponse<User[]>>('/users')).data.data
    }
  } catch {
...
<template>
  ...
  <label>{{ t('spa.competitions.year') }} <input v-model.number="year" type="number" @change="load"></label>
  ...
  <DataTable v-else :value="competitions" striped-rows>
    ...
    <Column field="description" :header="t('spa.competitions.description')" />
    ...
    <Column v-if="auth.isAuthenticated" :header="t('spa.competitions.created')">
      <template #body="{ data }">{{ userLabel(data.created) }}</template>
    </Column>
    <Column v-if="auth.isAuthenticated" :header="t('spa.competitions.updated')">
