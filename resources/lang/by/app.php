<?php

return [
    'club' => [
        'name' => 'Клюб',
        'persons_count' => 'Колькасць членаў',
    ],
    'persons' => [
        'title' => 'Члены федэрацыі',
    ],
    'navbar' => [
        'clubs' => 'Клюбы',
        'cups' => 'Кубак',
        'persons' => 'Удзельнікі',
        'competitions' => 'Спаборніцтвы',
        'flags' => 'Меткі этапаў',
        'help' => 'Даведка',
        'faq' => 'FAQ',
        'api' => 'API',
        'ranks' => 'Разрады',
    ],
    'ident' => [
        'edit' => [
            'title' => 'Звязаць выник и спартоўца',
        ]
    ],
    'distance' => [
        'length' => 'км',
        'points' => 'КП',
    ],
    'ranks' => 'Разрады',
    'rank' => [
        'recompleted_date' => 'Дата паследняга падтвярджэння',
        'completed_date' => 'Дата выканання',
        'finished_date' => 'Дата заканчэння',
        'check' => 'Праверка разрадаў',
        'check_button' => 'Праверыць',
        'list' => 'Спіс',
    ],

    'common' => [
        'name' => 'Імя',
        'lastname' => 'Прозвішча',
        'fio' => 'Прозвішча, Імя',
        'events_count' => 'Колькасць стартаў',
        'birthday' => 'Дата нараджэння',
        'login' => 'Увайсці',
        'sign-in' => 'Уваход',
        'sign-out' => 'Выхад',
        'registration' => 'Рэгістрацыя',
        'create' => 'Захаваць',
        'update' => 'Абнавіць',
        'new' => 'Дадаць',
        'delete' => 'Выдаліць',
        'cancel' => 'Адмена',
        'add_flags' => 'Рэдагаваць меткі',
        'title' => 'Назва',
        'description' => 'Апісанне',
        'back' => 'Назад',
        'date' => 'Дата',
        'dates' => 'Даты',
        'competitors' => 'Колькасць удзельнікаў',
        'edit' => 'Рэдагаваць',
        'group' => 'Группа',
        'groups' => 'Группы',
        'result' => 'Вынік',
        'place' => 'Месца',
        'points' => 'Ачкі',
        'rank' => 'Разрад',
        'complete_rank' => 'Выкананы разрад',
        'search' => 'Пошук',
        'year' => 'Год',
        'birthday_year' => 'Год нараджэння',
        'event_number' => 'Колькасць этапаў',
        'save' => 'Захаваць',
        'complete' => 'Выканаў',
        'time' => 'Час',
        'type' => 'Тып',
        'sum' => 'Аб\'яднаць',
        'do' => 'да',
        'url' => 'Спасылка',
        'close' => 'Зачыніць',
        'warn' => 'Увага',
        'delete_warn' => 'Сапраўды выдаліць? Гэта дзеянне будзе нельга скасаваць',
        'flag' => 'Метка',
        'average' => 'Сярэдняе',
        'is_equal' => 'Супадзенне',
        'has_in_base' => 'Ёсць у базе',
        'vk' => 'Па-за конкурсам',
        'last_payment' => 'Апошняя аплата',
        'cache_clear' => 'Абнавіць кэш',
    ],

    'competition' => [
        'title' => 'Спаборніцтвы',
        'add' => 'Даданне спаборніцтваў',
        'description' => 'Апісанне',
        'name' => 'Назва спаборніцтваў',
        'from_date' => 'Дата пачатку',
        'to_date' => 'Дата заканчэння',
        'add_competition' => 'Дадаць спаборніцтвы',
        'add_event' => 'Дадаць этап',
        'sum' => 'Аб\'яднаць этапы',
    ],

    'protocol' => 'Пратакол',
    'protocol-hint' => 'Если не добавлять файл, то обновления участников не будет',
    'cups' => 'Кубкі Федэрацыі',

    'cup' => [
        'add' => 'Даданне кубка',
        'edit' => 'Рэдагаванне кубка',
        'name' => 'Назва кубка',
        'events' => 'Этапы',
        'table' => 'Табліца',
        'events_count' => 'Колькась заліковых стартаў',
        'last_date' => 'Дата апошняга этапа',
        'type' => [
            'elite' => 'Элітны',
            'master' => 'Ветеранскі',
            'bike' => 'Веласіпедны',
            'sprint' => 'Спрынтарскі',
            'junior' => 'Юніёрскі',
            'youth' => 'Юнацкі',
        ],
    ],

    'lang' => [
        'ru' => 'Рус',
        'by' => 'Бел',
    ],

    'flags' => [
        'add_flags_title' => 'Редагаваць меткі этапа',
        'name' => 'Назва',
        'color' => 'Колер',
        'create' => [
            'title' => 'Дабаўленне новай меткі этапа',
        ],
        'edit' => [
            'title' => 'Рэдагаванне меткі этапа',
        ],
    ],

    'event' => [
        'flags' => 'Меткі',
        'title' => 'Этап',
        'add' => 'Даданне этапа',
        'edit' => 'Рэдагаванне этапа',
    ],

    'person' => [
        'create_button' => 'Дадаць асобу',
        'create_title' => 'Даданне асобы',
        'edit_title' => 'Рэдагаванне асобы',
    ],

    'faq' => [
        'description' => 'Апісанне працы сэрвісу',
        'create_competition' => 'Ствараем спаборніцтва',
        'add_events' => 'Дадаем дні спаборніцтва з пратаколамі (бярэм з orient.by)',
        'store_event' => 'Пратакол у выглядзе набора строк (нумар, Прозвішча, Імя, вынік, месца..) трапляе у базу',
        'parsing_start' => 'На пачатку кожнага часу пачынаецца працэс СУАДНОСІН усіх раней не "вызначаных" спартоўцаў з даданымі у сістему спартоўцамі (сатронка Члены Федэрацыі)',
        'relation_faq' => 'У выніку СУАДНОСІН некаторыя спартоўцы могуць быць не вызначанымі, хоць яны і ёсць у базе членаў федерацыі (напрыклад калі у пратаколе прозыішча або імя на іншай мове, або была змена прозвішча), тады есць магчымасць у пратаколе побач з прозвішчам спартоўца націснуць кнопку "add" і абраць сапращднага спартоўца. Дадзены "выпадак" будзе захаваны і будзе улічвацца пры усіх дальнейшых СУАДНОСІН',
        'relation_edit' => 'Таксама ёсць магчымасць адрэдагаваць няправільныя СУАДНОСІНЫ',
        'flags_adding' => 'Да атрыманнага дня спаборніцтв трэба прымацаваць адпаведныя меткі (меткі тыпу спаборніцтв, тыпу дыстанцыі, меткі кубкаў и г.д), што патрэбна для агрэгацыі спаборніцтв для атрымання статыстыкі па пэўнай прыкмеце',
        'api_link' => 'Дадзенныя магцыма выгрузіць па',
        'dev_plan' => 'План распрацоўкі',
        'dev_protocol' => 'Самая галоўная праца па даданню новых и старых пратаколаў и дапрацоўцы ПАРСЕРАЎ пад ніх',
        'dev_relation' => 'Праверка працы ІДЭНТЫФІКАТАРА (суаднісіны строк пратакола да рэальных людзей), и яго дапрацоўка',
        'dev_ui' => 'Дапрацоўка карыстальніцкага інтэрфейса дзеля зручнасці прагляду інфармацыі па спаборніцтвам, людзям, клубам',
        'dev_contact_us' => 'Даданне формы зваротнай сувязі',
        'dev_exports' => 'Даданне ЭКСПОРТАЎ са старонак пратаколаў у фармаце csv і xlsx',
    ],

    'up' => 'Уверх',
    'all' => 'Усё',

    'errors' => [
        'title' => 'Памылачка',
        'error' => 'Здарылася непрадбачанная памылка',
        '404error' => 'Такога пуці не існуе',
        'description' => 'Падробнае апісанне памылкі адпраўлена распрацоўніку',
        'description404' => 'Пачніце спачатку',
        'support' => 'Aбо скарыстайцеся сэрвісам падтрымкі',
        'next' => 'Ў бліжэйшы час памылка будзе выпраўлена',
        'thanks' => 'Дзякуй за тое што карыстаецеся нашым сервісам',
    ],

    'api' => [
        'methods' => 'Метады Api',
        'competitions_list' => 'Ліст спаборніцтваў',
        'events_list' => 'ліст этапаў спаборніцтваў',
        'results_list' => 'Вынікі этапа',
        'clubs_list' => 'Ліст клюбаў',
        'athletes_list' => 'Ліст спартоўцаў',
    ],

    'registration' => [
        'title' => 'Регістрацыя праз емейл',
        'email' => 'На Ваш адрас электроннай пошты адпраўлен ліст з інструкцыяй для працягнення рэгістрацыі',
        'success' => 'Паспяховая рэгістрацыя',
        'send-email' => 'Ваш пароль высланы на Вашу пошту',
    ],

    'groups' => [
        'events_count' => 'Колькасць спаборніцтваў',
    ],

    'table' => [
        'rows_count' => 'Запісаў на старонку',
        'nothing_found' => 'Нічога не знойдзена',
        'items' => 'Запісы з',
        'po' => 'па',
        'iz' => 'з',
    ]
];
