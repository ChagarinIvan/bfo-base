...
Для query/search DTO сначала используй паттерн `SearchPersonDto` или `SearchClubDto`:

- правила валидации объявляются в `requestValidationRules()`;
- `normaliseRequestData()` делает только необходимый `trim` строковых поисковых полей и удаляет
  пустые значения;
- `fromArray()` заполняет поля простыми приведениями `(string)`, `(int)` и `(bool)` после валидации;
- не добавляй `filter_var`, отдельные type-extractor-ы, универсальные normalizer-ы и ручную
  обработку transport-типов, если для этого нет отдельного устоявшегося проектного паттерна.

Перед каждым изменением API этот skill ОБЯЗАТЕЛЬНО перечитывается, а новый DTO сверяется минимум
с двумя существующими DTO того же назначения.

## Стиль условных операторов

Каждый `if` всегда оформляется фигурными скобками, включая однострочные условия. Синтаксис
`if ($condition) return;` и аналогичные однострочные формы не используется.
